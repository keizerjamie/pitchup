      ...
      {hasHistory && (
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/50">
            <h2 className="font-semibold text-gray-800">{t.periodization.history}</h2>
          </div>
          <div className="divide-y divide-gray-100">
            {history.map((item) => (
              <div key={item.eventId} className="px-5 py-3.5 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-gray-900 text-sm">{formatDate(item.date, t.browserLocale)}</div>
                  <div className="text-xs text-gray-400 mt-0.5">
                    {METING_CATEGORIES.map((c) => item.steps[`${c.key}_stap` as keyof MetingSteps]).join(' · ')}
                    {item.notes && <span className="ml-2 text-gray-500 italic">{item.notes}</span>}
                  </div>
                </div>
                {item.eventId === latestId && (
                  <button
                    ...
                    className="text-xs font-semibold text-brand px-3 py-1.5 rounded-lg border border-gray-200 hover:border-brand/40 transition-colors flex-shrink-0"
                  >
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-lg hover:bg-red-50 flex items-center justify-center text-gray-400 hover:text-red-500 transition-colors flex-shrink-0"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[92dvh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <p className="text-sm text-gray-500">{t.periodization.hint}</p>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t.periodization.date}</label>
                <input
                  type="date"
                  value={date}
                ...
                {METING_CATEGORIES.map((cat) => {
                  ...
                  return (
                    <div key={cat.key} className="p-4 flex items-center gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-gray-900 text-sm">{t.periodization.categories[cat.key] ?? cat.label}</div>
                        <div className="text-xs text-gray-400 mt-0.5">{t.periodization.maxSteps.replace('{n}', String(cat.maxStap))}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-14 text-center px-2 py-1.5 rounded-lg border border-gray-200 focus:outline-none focus:border-accent focus:ring-2 focus:ring-brand-light font-semibold text-gray-900"
                        />
                        <button
                          type="button"
                          onClick={() => changeStep(stepKey, 1, cat.maxStap)}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t.event.notes}</label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder={t.event.notesMeetingPlaceholder}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:border-accent focus:ring-2 focus:ring-brand-light text-gray-900 placeholder-gray-400 resize-none text-sm"
                />
              </div>
            </div>
