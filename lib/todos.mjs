...
/** De taaktypes die per event kunnen voorkomen. */
export const TASK_TYPES = ['squad', 'lineup', 'analysis', 'training_plan']

/**
 * Venster-constante voor zichtbaarheid vooruit (in dagen, t/m +7 = dezelfde
 * weekdag volgende week). Er is bewust GEEN retentie-venster terug: een
 * afgeronde taak verdwijnt zodra zijn deadline verstreken is (zie isTaskVisible).
 */
 ...
 *
 * Eén regel voor open én afgeronde taken: een taak waarvan de deadline
 * verstreken is (gisteren of eerder) verdwijnt van de hoofdpagina. Alleen
 * taken met een deadline vandaag of later blijven staan. Voor
 * squad/lineup/training_plan is de deadline de event-dag zelf; alleen de
 * analyse heeft een eigen deadline (eerste training ná de wedstrijd).
 *
 * @param {object} p
 * @param {string} p.taskType         'squad' | 'lineup' | 'analysis' | 'training_plan'
...
export function isTaskVisible({ taskType, done, daysUntilEvent, daysUntilDeadline }) {
  const daysUntilOwnDeadline = taskType === 'analysis' ? daysUntilDeadline : daysUntilEvent
  // Deadline verstreken → weg, ongeacht open of afgerond.
  if (daysUntilOwnDeadline < 0) return false
  if (taskType === 'analysis' && !done) {
    // Open analyse: pas vanaf de wedstrijddag, en GEEN +7-cap op de deadline
    // (never-miss, ook als de eerstvolgende training ver weg ligt).
    return daysUntilEvent <= 0
  }
  // Overig (open squad/lineup/training én alles wat afgerond is): t/m het
  // forward-venster op de event-dag.
  return daysUntilEvent <= FORWARD
}
