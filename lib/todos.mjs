 ...
 * RETENTION is bewust 6 en niet 7: een afgeronde taak van dezelfde weekdag
 * vorige week (dag -7) is voor de trainer niet meer relevant en valt er dus
 * net buiten. Vooruit blijft het venster t/m +7 (dezelfde weekdag volgende week).
 */
export const FORWARD = 7
export const RETENTION = 6
...
export function isTaskVisible({ taskType, done, daysUntilEvent, daysUntilDeadline }) {
  if (done) {
    // Afgerond: kort zichtbaar rond de event-dag (t/m RETENTION dagen terug,
    // t/m FORWARD dagen vooruit).
  ...
  // Open squad/lineup/training: forward-venster; vervalt na de eigen event-dag.
